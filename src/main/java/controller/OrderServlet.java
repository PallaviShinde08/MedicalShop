...
public class OrderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        out.println("<html><head><title>Online Medical Shop - Orders</title>");
        ...
        try (Session dbSession = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (orders.isEmpty()) {
                out.println("<p>You have no orders yet. <a href='/medicines'>Shop now</a>.</p>");
            } else {
                ...
                for (Order order : orders) {
                    ...
                    for (OrderItem item : order.getOrderItems()) {
                        out.println("<p>" + item.getMedicine().getName() + " - " + item.getQuantity() + " x $" + item.getUnitPrice() + "</p>");
                    }
